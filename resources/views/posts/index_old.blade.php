...
@section('title', 'All Posts | ' . config('app.name', 'SoulVerse'))
@section('description', 'Explore all the creative stories, experiences, and thoughts shared by our community of writers.')
        ...
        <div class="navbar-content">
            ...
            <ul class="navbar-nav">
                ...
                @auth
                    ...
                    <li><a href="{{ route('posts.create') }}" class="btn btn-primary">
    ...
    <section class="filters-section">
        <div class="filters-content">
            <div class="filter-group">
                <a href="{{ route('posts.index') }}" class="filter-btn {{ !request('category') && !request('arc') ? 'active' : '' }}">
                    All Posts
                </a>
                @if(isset($categories) && $categories->count() > 0)
                    ...
                    @endforeach
                @endif
                @if(isset($arcs) && $arcs->count() > 0)
                    @foreach($arcs as $arc)
                        <a href="{{ route('posts.index', ['arc' => $arc->slug ?? $arc->id]) }}" 
                           class="filter-btn {{ request('arc') === ($arc->slug ?? $arc->id) ? 'active' : '' }}">
                            <svg class="icon-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="posts-container">
            <div class="posts-grid" id="postsGrid">
                ...
                @forelse($posts as $post)
                    <article class="post-card" data-category="{{ $post->category ?? 'uncategorized' }}" data-title="{{ strtolower($post->title) }}">
                        @if($post->cover_image)
                ...
                @empty
                    <div class="empty-state">
                        ...
                        <p class="empty-description">
                            @if(request('search') || request('category') || request('arc'))
