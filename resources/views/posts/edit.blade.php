...
@section('title', 'Edit Post | ' . config('app.name', 'SoulVerse'))
@section('description', 'Edit your post: ' . $post->title)
        ...
        <div class="form-container bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8">
            ...
            <form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ old('title', $post->title) }}" required>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="category" id="category" class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ old('category', $post->category) }}" placeholder="e.g., Life Arc, Tech">
                    </div>
                    <div>
                        ...
                        <select name="arc_id" id="arc_id" class="w-full rounded-md border-gray-300 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            ...
                            @foreach($arcs as $arc)
                                <option value="{{ $arc->id }}" {{ old('arc_id', $post->arc_id) == $arc->id ? 'selected' : '' }}>{{ $arc->title }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    @if ($post->cover_image)
                        <div class="mb-4">
                            <img src="{{ asset('storage/' . $post->cover_image) }}" alt="Current cover image" class="rounded-lg max-w-xs">
