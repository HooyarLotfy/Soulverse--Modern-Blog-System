...
@section('title', 'Edit Arc | ' . config('app.name', 'SoulVerse'))
@section('description', 'Update the details for the story arc: ' . $arc->title)
...
<main class="bg-gray-50 dark:bg-gray-900">
    <div class="container" style="max-width: 800px; margin: 6rem auto 3rem; padding: 0 1rem;">
        <div class="arc-form-container" style="background: white; border-radius: 1rem; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.05); padding: 2.5rem; margin-bottom: 2rem;" class="dark:bg-gray-800 dark:shadow-2xl">
            ...
            <form action="{{ route('arcs.update', $arc) }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 1.5rem;">
                @csrf
                @method('PATCH')
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-control" rows="4" required style="width: 100%; padding: 0.75rem 1rem; border: 1px solid var(--gray-300); border-radius: 0.5rem; font-size: 1rem; transition: all 0.3s ease; color: var(--gray-800); resize: vertical;" class="dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400">{{ old('description', $arc->description) }}</textarea>
                    @error('description')
                        <div style="color: var(--error); font-size: 0.875rem; margin-top: 0.5rem;">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    @if ($arc->cover_image)
                        <div class="mb-4">
                            <img src="{{ asset('storage/' . $arc->cover_image) }}" alt="Current cover image" class="rounded-lg max-w-xs">
                ...
                <div style="display: flex; gap: 1rem; margin-top: 1.5rem; justify-content: flex-end;">
                    <a href="{{ route('arcs.show', $arc) }}" class="btn btn-secondary">Cancel</a>
